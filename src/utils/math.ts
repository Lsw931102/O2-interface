 ...
 * @returns
 */
export const numberFormat = (n: any, cutZero = false, shortNum = 18): string => {
 ...
 * @return {*} 百分比
 */
export const convPercentage = (molecular: number, denominator: number, isInt = true) => {
 ...
 * @return {*}
 */
export const isLessThanMin = (min = 0.01, amount: number, fixed = 0, isFormat = false) => {
  return amount < min
    ? `<${min}`
    : isFormat
...
export function formatNumFloat(number: any, digits = 0, decPoint = '.') {
  if (number) {
    ...
    if (floatNum?.length > 0) {
      return new BigNumber(number).toFixed(Math.min(floatNum?.length, digits), 1)
    }
    return number
  }
  return ''
...
export const calculationTokens = (
  ...
  if (!new BigNumber(token0Liqudity).eq(0) && !new BigNumber(token1Liqudity).eq(0)) {
    ...
    if (new BigNumber(token0Liqudity).eq(0)) {
      ...
      // 计算token1的数量(总债务乘以(100 - token0的占比)后，除以token1的价值)
      const token1Result = new BigNumber(totalDebt).div(new BigNumber(token1Price)).toString()
      return {
        token0Result,
        token1Result,
      }
      ...
      // 计算token0的数量(总债务乘以token0的占比后，除以token0的价值)
      const token0Result = new BigNumber(totalDebt).div(new BigNumber(token0Price)).toString()
      // token1的数量=0
...
export const calculationMaxToken = (
  ...
  if (activeToken === 0) {
    ...
    newTokenRatio = new BigNumber(token0Result).times(token0Price).div(totalDebt).toFixed(2)
  }
  // 选择最大为token1
  else {
    token1Result = new BigNumber(token1Liqudity).toString()
